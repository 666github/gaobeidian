...
var statDataList=[];
var colorMark=[];
var colorList=["lightsalmon","lightseagreen","DeepPink","LimeGreen","orange","darkorchid"];
//初始化
function WSinit(){
...
function StatisticApp(){
	...
	this.loadData=function(_startDate,_endDate){
        ...
        //获取统计结果
        websocket.send("JsonGetFWStatByZGLCRequest", $.extend(userData, { }), function (response) {
...
//		}
	...
	function getHousePieOption(_typeId,_showtypeArr,_action){
		...
		for(var x=0;x<statDataList.length;x++){
			var value=_action? statDataList[x].COUNT:statDataList[x].MJ;
			var dataValue={
				value:value,
				name: statDataList[x].FL,
				itemStyle: {
					normal: {color:colorList[x%colorList.length]}
				},
			};
