...
function StatisticApp(){
	...
	this.loadData=function(_startDate,_endDate){
        ...
        //获取统计结果
        websocket.send("JsonGetFWStatByBuildYearRequest", $.extend(userData, { }), function (response) {
            if (response.status == 'success') {
				...
				new AlertBoxMessage(
					{title:"信息提示",
					 message:"请求过程中遇到了错误："+response.message,
...
//		}
	...
	function getHouseBarOption(_showtypeArr,_action){
		...
		var option={
            "color":[_action?"lightseagreen":"lightseagreen"],
			"tooltip": {
				"trigger": "axis",
				"axisPointer": {
					"type": "shadow"
