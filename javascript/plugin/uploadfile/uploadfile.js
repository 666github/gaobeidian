...
function UpLoadfile(_config){
	...
	this.setServerInterface=function(_url) {
		config.serverInterface=_url;
	};
	//绑定上传按钮
...
function UpLoadfileAsBinary(_config){
	var content=this,callback=null,bindObject=null;
	var defaultConfig={
		//服务端接收文件路径
		serverInterface:"",
	...
	function creatFileInput(){
		...
		var file=document.createElement("input");
		file.setAttribute("type","file");
		file.setAttribute("name",config.filename);
		file.setAttribute("accept","*.*");
		file.style.position="absolute";
		file.style.width="100%";
		file.style.height="100%";
	...
	this.submit=function(data){
		...
		fileReader.onerror=function() {
			 callfun("errorEvent",getResult({errorCode:2,errorinfo:"上传失败!"}));
		}
		return true;
	};
	//选择了文件事件
