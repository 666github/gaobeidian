...
插件: 选择框
作者: 马腾
日期: 2015-08-04 21:05:33
操作机制:系统在网页载入时会自动进行初始化操作，无需重复初始化，如动态创建了此组件，可单独进行初始化，相关API如下！
默认选择:添加 selected 属性 如:<li data-value="1" selected>北京</li>

初始化:		$("#selectbox").selectBox().selectBox("callback",function(_data,_caption,_selectbox,_selectchild){})
删除全部:		$("#selectbox").selectBox("removeAll")
删除一项:		$("#selectbox").selectBox("removeForIndex",索引)
选择一项:		$("#selectbox").selectBox("selectForIndex",索引)
添加一项:		$("#selectbox").selectBox("add","插入项","项目值",是否选中,是否触发选中事件)
显示下拉:		$("#selectbox").selectBox("showList")
隐藏下拉:		$("#selectbox").selectBox("hidelist")

设置标题和值:
$("#selectbox").selectBox("setCaption","标题","值")

获取下拉列表长度:	
$("#selectbox").selectBox("getLength")

设置某一项值:	
$("#selectbox").selectBox("setDataValue",索引,"标题",值)

获取选中元素的Index：
$("#selectbox").selectBox("getSelectIndex")

添加某一项索引 前 添加一项（索引，内容，值，是否选择,是否触发选中事件）
$("#selectbox").selectBox("addBeforeIndex",索引,"标题","值",true,是否触发选中事件)

添加某一项索引 后 添加一项（索引，内容，值，是否选择,是否触发选中事件）
$("#selectbox").selectBox("addAfterIndex",索引,"标题","值",true,是否触发选中事件)

通过索引删除列表（索引）
$("#selectbox").selectBox("removeForIndex",索引)

通过值删除列表（值，允许删除多项）
$("#selectbox").selectBox("removeForData","值",允许删除多项_true)

获取当前选中值 返回{caption:"",data:""}
$("#selectbox").selectBox("getForIndex",索引)

用Index选中某个元素
$("#selectbox").selectBox("selectForIndex",索引,是否触发选中事件)
通过值选中某个元素
$("#selectbox").selectBox("selectForData","值",是否触发选中事件)

设置回调函数:
$("#selectbox").selectBox("callback",function(_data,_caption,_selectbox,_selectchild){})

弹出收回回调函数
$("#selectbox").selectBox("showCallback",function(_show,_data,_caption,_selectbox,_selectchild){})

扩展属性: 
	可以在主标签 <div class="selectbox"></div>上添加扩展属性，为第一优先级
	如:<div class="selectbox" data-height="30" data-width="200"></div>
	扩展属性值必须使用双引号，扩展属性对照表如下:
		data-default-value		什么都没有选择时候的默认文本 如：请选择
		data-width				组件宽度
		data-height				组件高度
		data-body-width			列表宽度
		data-body-height		列表高度
...
<div class="selectbox selectbox_width200">
	...
	<div class="selectbox_value">请选择</div>
	<div class="selectbox_list selectbox_width200">
		...
		<ul class="selectbox_body">
			<li data-value="0">1所有</li>
			<li data-value="0" selected>自动选择此项</li>
			<li data-value="0">3分类名称</li>
			<li data-value="0">4暂无分类</li>
			<li data-value="0">5关闭话题</li>
			<li data-value="0">6暂无分类</li>
			<li data-value="0">7关闭话题</li>
		</ul>
	</div>
</div>
...
(function($){
	...
	*/
	...
	function onNodeMouseover(event){
		var node=((!$(event.target).parent().hasClass("selectbox_body"))?($(event.target).parentsUntil(".selectbox_body").last()):$(event.target));
		if(!node.hasClass("selectbox_selected")){
			node.addClass("selectbox_nodebover");
		}
	}
	function onNodeMouseout(event){
		((!$(event.target).parent().hasClass("selectbox_body"))?($(event.target).parentsUntil(".selectbox_body").last()):$(event.target)).removeClass("selectbox_nodebover");
	}
	...
	var fun={
		...
		,add:function(_value,_data,_select,_isTrigger){
			this.each(function(index,element){
				...
				if(_select==true){
					if(_isTrigger===false){
						$(element).find("input:hidden").text($(element).find(".selectbox_body").children().eq(index).attr("data-value"));
						$(element).find(".selectbox_value").text($(element).find(".selectbox_body").children().eq(index).text());
					}else{
		...
		,removeAll:function(){
			this.each(function(index,element){
				if($(element).data("selectboxInit")!=true)return;
				$(element).find(".selectbox_body").css("top","0").children().remove();
